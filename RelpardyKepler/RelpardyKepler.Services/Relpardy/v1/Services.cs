...
using RelpardyKepler.Interfaces.Relpardy.v1;
using RelpardyKepler.Interfaces.Relpardy.v1.Exceptions;
using RelpardyKepler.Interfaces.Relpardy.v1.Models;

namespace RelpardyKepler.Services.Relpardy.v1
{
    public class Services : IServices
    {
        ...
        public Services(IHelper helper, ILog logger)
        {
            // Note: Set the logging context to the current class.
            _logger = logger.ForContext<Services>();
            _helper = helper;
        }

        public async Task<ServicesModel> GetWorkspaceNameAsync(int workspaceID)
        {
            ServicesModel model;
            ...
            catch (Exception exception)
            {
                ...
                throw new ServicesException($"Workspace {workspaceID} not found.")
                {
                    FaultSafeObject = new ServicesException.FaultSafeInfo()
                    {
        ...
        public async Task<List<ServicesModel>> QueryWorkspaceByNameAsync(string queryString, int limit)
        {
            var models = new List<ServicesModel>();
            ...
            var proxy = _helper.GetServicesManager().CreateProxy<IServices>(ExecutionIdentity.CurrentUser);
            ...
            try
            {
                ...
                foreach (int workspaceID in workspaceIDs)
                {
                    ...
                    //       See also https://blog.stephencleary.com/2012/07/dont-block-on-async-code.html
                    //       Warning: Improper use of the tasks can cause deadlocks and performance issues within an application.
                    ServicesModel wsModel = await proxy.GetWorkspaceNameAsync(workspaceID).ConfigureAwait(false);
                    if (wsModel != null)
                    {
                        models.Add(wsModel);
                    }
            ...
            catch (Exception exception)
            {
                ...
                throw new ServicesException($"An exception occured during query for workspace(s) containing {queryString}.");
            }
